...
class XPlaneUdpLocationParserTest {
    ...
    @Test
    fun `parse computes speed and bearing from consecutive index 20 positions`() {
        var nowNanos = 0L
        val parser = XPlaneUdpLocationParser(nowNanos = { nowNanos })

        parser.parse(xplaneDataPacket(latitude = 50.0f, longitude = 14.0f, altitudeFeet = 1_000f))
        nowNanos = 10_000_000_000L
        val location = parser.parse(xplaneDataPacket(latitude = 50.0f, longitude = 14.01f, altitudeFeet = 1_000f))

        assertNotNull(location)
        assertEquals(71.47f, location.speedMetersPerSecond, absoluteTolerance = 0.2f)
        assertEquals(89.996f, location.bearingDegrees, absoluteTolerance = 0.01f)
    }
