...
/**
 * Computes SHA-256 hashes used for package integrity checks.
 */
class Sha256Hasher {
    ...
    private fun pad(input: ByteArray): ByteArray {
        ...
        val oneByte = 1
        val lengthBytes = 8
        val padding = ((64 - ((input.size + oneByte + lengthBytes) % 64)) % 64)
        val result = ByteArray(input.size + oneByte + padding + lengthBytes)

        input.copyInto(result, 0)
